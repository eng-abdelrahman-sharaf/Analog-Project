...
fprintf('Playing sound: Filtered baseband signal\n');
...
fprintf('Playing sound: Envelope detected DSB-TC\n');
sound(env_tc_ds,Fs); pause(3);

fprintf('Playing sound: Envelope detected DSB-SC\n');
...
for snr = SNRs
    ...
    fprintf('Playing sound: DSB-SC coherent demodulation (SNR = %d dB)\n', snr);
...
fprintf('Playing sound: DSB-SC with frequency offset\n');
sound(resample(freq_err,Fs,Fs_new),Fs); pause(3);

fprintf('Playing sound: DSB-SC with phase error (20 degrees)\n');
...
fprintf('Playing sound: SSB coherent demodulation\n');
...
fprintf('Playing sound: Practical SSB demodulated signal\n');
...
for snr = SNRs
    ...
    fprintf('Playing sound: SSB coherent demodulation (SNR = %d dB)\n', snr);
    sound(demod_ds, Fs); pause(3);
end
...
fprintf('Playing sound: SSB-TC envelope detector output\n');
...
fprintf('Playing sound: NBFM demodulated signal\n');
sound(NBFM_ds, Fs);
